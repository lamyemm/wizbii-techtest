...
const Post = ({profile, tags, postContent, thanx, comments}) => {
  console.log(profile, tags, postContent, thanx, comments)
  ...
  return (
    <div className="wrapper">
      ...
      <div className="eight">
        <span className="specialtext">{comments}  {comments !== 0 ? (<span>commentaires</span>):<span>commentaire</span>} </span>- {thanx} Thanx
      </div>
      <div className="nine">
        <hr />
      </div>
      <div className="ten">Ajouter un commentaire...</div>
      <div className="eleven">Tu vois ceci car tu es abonné au tag #Marketing</div>
    </div>
  )
}
